...
class print_list {
 ...
 public:
  print_list(const unsigned&sz = 1, const unsigned&wid = 1, const unsigned&len = 1, const unsigned&cnt = 0);  // constructor
  void operator()(const T&);
};
template<typename T>
print_list<T>::print_list(const unsigned &s, const unsigned &w, const unsigned &l, const unsigned &c)	 //defining the constructor
{
	sz = s;
	wid = w;
	len = l;
...
template<typename T>
void print_list<T>::operator()(const T &x)					//implementing the print_list function which would..print the contents
{
	cout.width(wid); 
	cout.precision(2);
