...
void get_list(vector<T> &v, const char *path) 
{
  ...
  ifstream f;						//f is the ifstream variable to open file for input
  f.open(path);						//open file
  T num;						//variable to hold my number value
  if (!f.is_open())
  {
	  cout << "I Am Sorry, But I Have Failed To Open" << endl;
  }
  v.clear();
  while (f >> num)					//reading the number value into the file
  {
	  v.push_back(num);				//adding the number value to my vector
  }
...
{
  // Your implementation goes here!!!
  make_heap(v.begin(), v.end(), pred);			//creates heaps according to pred
  sort_heap(v. begin(), v.end(), pred);			//sorts heaps according to pred
}
